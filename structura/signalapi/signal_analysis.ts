...
export class SignalProcessor {
  ...
  /**
   * Aggregate signals by id, collecting all per id.
   */
  groupById(signals: Signal[]): Record<string, Signal[]> {
    return signals.reduce((acc, s) => {
  ...
  summarize(signal: Signal): string {
    ...
    return `[${time}] ${signal.type.toUpperCase()}(${signal.id}): ${JSON.stringify(
      signal.payload
    )}`
  }

  /**
   * Generate a summary report for multiple signals.
   */
  ...
  /**
   * Compute statistics from a batch of signals.
   */
  stats(signals: Signal[]): { total: number; types: Record<string, number> } {
    return {
      total: signals.length,
      types: this.aggregateByType(signals),
    }
  }
}
